...
export class AuthResolver {
  ...
  async login(@Args('login') login: string, @Args('password') password: string): Promise<LoginResponseDto> {
    try {
      ...
      if (!result) throw new CustomGraphQLError('Invalid request', 400)

      const { status, data } = result
      if (status !== 200 || !data.accessToken)
        throw new CustomGraphQLError('Invalid login credentials. Authentication required.', result.status)

      const { accessToken, refreshToken, expiresAt } = data
      return { status, accessToken, refreshToken, expiresAt }
    } catch (error) {
  ...
  async refreshToken(
    ...
    const { status, data } = result
    const { accessToken, refreshToken: newRefreshToken, expiresAt } = data

    return { status, accessToken, refreshToken: newRefreshToken, expiresAt }
  }
  ...
  async logout(@Context() context: { req: ContextReq; res: Response }) {
    ...
    try {
      ...
      return { status: revoked.status, revoked: revoked.status === 204 }
    } catch (error) {
  ...
  async securedQuery(@Context() context: { req: ContextReq }) {
    const user = context.req.user
